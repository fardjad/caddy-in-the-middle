...
CITM supports deployment as a standalone gateway or as a sidecar proxy. The
following architecture demonstrates a mixed-mode deployment.
...
- **OpenSSL** (to generate a custom Root CA for testing)
...
- A Root CA certificate (`rootCA.pem`) and key (`rootCA-key.pem`) within a
  designated directory. Self-signed certificates can be generated via OpenSSL:

  ```bash
  mkdir -p certs && cd certs
  openssl req -x509 -newkey rsa:4096 -keyout rootCA-key.pem -out rootCA.pem \
    -days 365 -nodes -subj "/C=NL/ST=State/L=City/O=Organization/CN=CITM-RootCA"
  ```
